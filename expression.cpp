...
Include::Include(string header, bool system) : header_name(header), is_system_header(system) {}
Include::~Include() {}
...
Type::Type(string name) : type_name(name), is_pointer(false), is_array(false), array_size(nullptr), is_reference(false) {}
Type::Type(string name, bool pointer, bool is_reference) : type_name(name), is_pointer(pointer), is_array(false), array_size(nullptr), is_reference(is_reference) {}
Type::Type(string name, Exp *size) : type_name(name), is_pointer(false), is_array(true), array_size(size), is_reference(false) {}
Type::~Type()
{
    if (array_size)
        delete array_size;
}
Type::Type(string name, bool pointer, bool array, Exp *size, bool is_reference) : type_name(name), is_pointer(pointer), is_array(array), array_size(size), is_reference(is_reference) {}
...
AssignExp::AssignExp(Exp *l, Exp *r, BinaryOp op) : left(l), right(r), assign_op(op) {}
AssignExp::~AssignExp()
{
    delete left;
    delete right;
...
UnaryExp::UnaryExp(Exp *uexp, UnaryOp op, bool prefix) : uexp(uexp), op(op), is_prefix(prefix) {}
...
MemberAccessExp::MemberAccessExp(Exp *obj, string member, bool ptr_access) : object(obj), member_name(member), is_pointer(ptr_access) {}
...
IfStatement::IfStatement(Exp *cond, Body *statements, Stm *elsChain) : condition(cond), statements(statements), elsChain(elsChain)
{
...
ElseIfStatement::ElseIfStatement(Tipo t, Exp *cond, Body *body, Stm *nextChain, int id, int parentId) : tipo(t), condition(cond), body(body), nextChain(nextChain), id(id), parentId(parentId) {}
...
ForStatement::ForStatement(VarDec *init, Exp *condition, Exp *update, Body *b) : init(init), condition(condition), update(update), b(b)
{
    static int counter = 1;
    id = counter++;
}
...
VarDec::VarDec(std::vector<Type *> types, vector<string> vars) : types(types), vars(vars) {}
...
GlobalVarDec::GlobalVarDec(Type *type, string name, Exp *init) : type(type), var_name(name), initializer(init) {}
...
GlobalVarDecList::~GlobalVarDecList()
{
    for (auto vardec : global_vardecs)
        delete vardec;
}
...
Function::Function(Type *ret_type, string name, ParameterList *params, Body *body) : return_type(ret_type), name(name), parameters(params), body(body) {}
Function::~Function()
{
    delete return_type;
    delete parameters;
...
StructDeclaration::StructDeclaration(string name, VarDecList *members) : struct_name(name), members(members) {}
...
Program::Program(IncludeList *includes,
                 ...
                 MainFunction *main) : includes(includes),
